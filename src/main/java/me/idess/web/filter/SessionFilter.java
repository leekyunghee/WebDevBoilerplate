	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse,
		...
		try {
			...
			if (request.getParameter("q") != null) {
				logger.debug("q data is not null");
				String q = request.getParameter("q").toString();
				JSONObject jsonObject = JSONObject.fromObject(q);
				...
				if (httpSession.getAttribute("Token") != null) {
					...
					if (TokenObject.getToken(httpSession.getAttribute("Account").toString()) != null
							&& TokenObject.getToken(httpSession.getAttribute("Account").toString()).equals(httpSession.getAttribute("Token").toString())) {
						logger.debug("토큰 유효");
				...
				//세션이 존재하지 않으면
					...
					if (serviceCode.equals("loginServiceCode") || uri.equals("/login")) {
						//headerObject.put("Account", ((JSONObject)((JSONObject) bodyObject.get(Constants.Parameter)).get(Constants.LoginKey)).get(Constants.LoginId));
						headerObject.put("Account", ((JSONObject) bodyObject.get(Constants.Parameter)).get(Constants.LoginId));
					}
				}
			...
			if (httpSession.getAttribute("Token") != null) {
				...
				if (serviceCode.equals("loginServiceCode") || uri.equals("/login") || uri.equals("/") || uri.equals("/index.html")) {
					filterChain.doFilter(request, response);
				} else {	// 비정상적인 요청일 경우
					response.sendRedirect("/");
			...
			if ((serviceCode.equals("loginServiceCode") || uri.equals("/login"))&& request.getAttribute("q") != null) {
				String q = request.getAttribute("q").toString();
				JSONObject jsonObject = JSONObject.fromObject(q);
				JSONObject dataInfoObject = (JSONObject) jsonObject.get(Constants.DataInfo);
				JSONObject bodyObject = (JSONObject) dataInfoObject.get(Constants.Body);
				JSONObject resultDataObject = (JSONObject) bodyObject.get(Constants.ResultData);
				JSONObject loginInfo = (JSONObject) resultDataObject.get("Login");
				
				// 로그인 성공일 경우 세션 생성
				if (loginInfo != null && "Y".equals(loginInfo.get("successYn"))) {
					httpSession.setAttribute("Account", loginInfo.get(Constants.LoginId));
					httpSession.setAttribute("Token", loginInfo.get("token"));
					logger.debug("login success ; add session > Token["+httpSession.getAttribute("Token")+"]");
				}
			}
